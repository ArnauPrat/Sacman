...
atTiledLevel* LoadTiledLevel( const char* fileName ) {
    ...
    for( Json::Value::iterator it = layers.begin(); it != layers.end(); ++it) {
        ...
        if( std::strcmp(type,"tilelayer") == 0 ) {
            ...
            level->m_Layers[i].m_TileLayer.m_Cells = new atTiledCell[ level->m_Layers[i].m_TileLayer.m_NumCells ];
            int j = 0;
            int k = 0;
            ...
            level->m_Layers[i].m_Type = E_OBJECT_GROUP;
            const Json::Value& objects = (*it)["objects"];
            level->m_Layers[i].m_ObjectGroup.m_Objects = new atTiledObject[objects.size()];
            int j = 0;
            for( Json::Value::iterator it2 = objects.begin(); it2 != objects.end(); ++it2) {
                atTiledObject& object = level->m_Layers[i].m_ObjectGroup.m_Objects[j];
                if( (*it2).isMember("gid") ) {
                ...
                if( object.m_NumProperties > 0 ) {
                    object.m_Properties = new atTiledObjectProperty[object.m_NumProperties];
                }
                int k = 0;
    ...
    level->m_TileSets = new atTiledTileSet[level->m_NumTileSets];
    i = 0; 
...
atVector2f TransformPosition( const atTiledLevel& level, int x, int y ) {
    atVector2f position = { x/ static_cast<float>(level.m_TileWidth), (level.m_Height*level.m_TileHeight-y) / static_cast<float>(level.m_TileHeight) };
    return position;
}
